...
- **Design-critique pass (v0.86.0)**: the Integrations page's "Sync Library" button now matches Overview's brand-purple styling; Library Health's KPI tiles use the same colored-icon-badge look as Overview's; Match Review's independent toggle filters (Covered/Suspicious/Packs/Singles only) are now pill-shaped and check-marked when active so they read as distinct from the status filters beside them; every horizontally-scrollable table shows a fade at the right edge only while there's more to scroll to, most noticeable on narrow screens; and the header breadcrumb dropped its redundant Area › Screen segment since the tab row directly below already shows the active screen. Overview's Push Failures / Failed Imports tiles now get a small amber marker when their count is nonzero
...
### Music — Artist Discovery (v0.39.0; refined v0.40.0–v0.42.0; UX cleanup + tray + Related Artists v0.48.0; Related Artists filters v0.49.0; candidate-pool fix + auto-add parity v0.87.0; optional embeddings v0.88.0–v0.88.1)
...
the mode outright rather than reporting a graph-only cycle as fully healthy. **v0.88.1** fixes a
