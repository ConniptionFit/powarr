...
- **Editable pack episode mappings (v0.15.0)**: the triage table's **Select Episode** dropdown picks any file from a reviewed season pack, and a pencil action opens an inline season/episode editor — corrections persist (`PUT /imports/{id}/pack-match`), are marked **user** confidence, and show green in the Pack Episodes column so manual overrides stand out from LLM guesses
