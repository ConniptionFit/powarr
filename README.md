...
- **Approved Queue batch (v0.29.0)**: Tautulli multi-user deletion protection (Settings → Cleanup); LLM temperature / max tokens / timeout overrides; CSV export on Cleanup tables; weekly ntfy digest; Compact/Comfortable row density; failed-import 30-day trend sparkline on the Dashboard. Per-library scoring profiles (#16) still awaiting confirmation.
