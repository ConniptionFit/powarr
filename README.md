...
- **Listen-before-add previews (v0.78.0)**: Artist Discovery and Related Artists cards gain an on-demand Preview button — embeds a YouTube video or plays a Spotify 30-second clip, whichever of the two you've configured in Settings → Integrations (both optional, both fail soft with "no preview available" rather than an error). YouTube Music has no official public API and isn't implemented
