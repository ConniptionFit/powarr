...
- **Persisted view state (v0.27.0)**: the Cleanup tab, Failed Imports view/sort/narrowing filters, and Deletion Suggestions min-score/type/mode/sort now survive a reload (browser localStorage, like the column layout has since v0.4.0)
...
- **Still queued + filter redesign (v0.35.0)**: Failed Imports primary views are **Needs attention** / **Still queued** / **History**. Still queued lists downloads that remain in an *arr queue even after Accept/Reject (Reopen puts them back in triage). Narrowing filters (search, platform, Covered, Suspicious, Packs) live behind a Filters toggle.
