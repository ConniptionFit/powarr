...
- **Instant music rescoring (v0.37.0)**: the same containment checks now shape the deterministic matcher directly — confirmed artist+album floors confidence at 0.88, junk uploader-tag matches drop to 0.35–0.45 — and a **Rescore Music** button re-runs matching on all open Lidarr/Readarr rows in seconds, no LLM required. Any rescore (deterministic or LLM) that lifts a row past the auto-resolve threshold queues it for import automatically (v0.37.1) — same pipeline, guards, and tray progress as clicking Accept.
