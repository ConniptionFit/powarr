...
| Field | Value |
|---|---|
| Version | v0.32.0 — per-*arr LLM guidance, Lidarr edition/CD strip, partial gap-fill Accept; v0.31.0 altTitles/junk/year/pack chunks; v0.30.0 cleanup scoring + LLM scaffold |
...
| Active Processes / Task Tracking | Background-operation progress tracking, the corner tray | `services/tasks.py`, `api/v1/tasks.py`, `frontend/src/context/TaskContext.tsx`, `frontend/src/components/ActiveProcessesTray.tsx`; [[Active Processes Tray]]. In-memory only, no DB. Events ride the existing `/api/v1/imports/events` bus (`import_matcher.publish()`), not a dedicated endpoint. v0.22.0; LLM task card renders `BotState` `thinking`/`complete`/`error` per status (v0.25.0); `import_batch` kind for async Accept Selected / Process N (v0.28.0) |
