...
| Active Processes / Task Tracking | Background-operation progress tracking, the corner tray | `services/tasks.py`, `api/v1/tasks.py`, `frontend/src/context/TaskContext.tsx`, `frontend/src/components/ActiveProcessesTray.tsx`; [[Planned - Active Processes Notification Tray]] (shipped-feature record despite the filename). In-memory only, no DB. Events ride the existing `/api/v1/imports/events` bus (`import_matcher.publish()`), not a dedicated endpoint. v0.22.0 |
...
| Task tracking / tray | [[Planned - Active Processes Notification Tray]] |
